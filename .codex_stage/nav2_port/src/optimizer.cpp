...
void Optimizer::getParams()
{
  ...
  getParam(s.amoeba_ancillary_debug, "amoeba_ancillary_debug", false);
  getParam(s.amoeba_shadow_mode, "amoeba_shadow_mode", true);
  getParam(s.amoeba_bias_enabled, "amoeba_bias_enabled", false);
  getParam(s.flow_critic_enabled, "flow_critic_enabled", false);
  getParam(s.amoeba_body_radius, "amoeba_body_radius", 2.5f);
  getParam(s.amoeba_debug_grid, "amoeba_debug_grid", false);
  getParam(s.amoeba_debug_arrow_spacing, "amoeba_debug_arrow_spacing", 0.40f);
  getParam(s.amoeba_debug_arrow_length, "amoeba_debug_arrow_length", 0.18f);
  getParam(s.amoeba_debug_arrow_width, "amoeba_debug_arrow_width", 0.015f);
  getParam(s.amoeba_mode, "amoeba_mode", std::string("flow"));
  ...
  if (s.amoeba_mode == "flow") {
    RCLCPP_INFO(
      logger_,
      "[AMOEBA] flow mode: water field over local costmap (reflood every %d "
      ...
      s.amoeba_body_radius, s.amoeba_bias_strength, s.amoeba_bias_gain,
      s.amoeba_shadow_mode ? "true" : "false",
      s.amoeba_bias_enabled ? "enabled" : "disabled",
      s.flow_critic_enabled ? "enabled" : "disabled",
      s.ancillary_collision_check ? "enabled" : "disabled");
  } else {
    RCLCPP_INFO(
      logger_,
      "[AMOEBA] ray mode: shape-conditioned sampling (scan=%.2fm arc=%.2frad "
      "rays=%d bias=%.2f)",
  ...
  const nav_msgs::msg::Path & plan, nav2_core::GoalChecker * goal_checker)
{
  ...
  critics_data_.flow_field =
    (settings_.amoeba_mode == "ray" || settings_.amoeba_shadow_mode ||
    !settings_.flow_critic_enabled) ?
    nullptr : &flow_field_;
}
...
void Optimizer::generateNoisedTrajectories()
{
  ...
  if (settings_.amoeba_mode == "ray") {
    if (settings_.amoeba_shadow_mode || !settings_.amoeba_bias_enabled) {
      computeAmoebaModes();
      if (settings_.amoeba_debug) {
        publishAmoebaDebug();
      }
    } else {
      applyAmoebaBias();  // legacy: seed shape-derived wrap modes before rollout
    }
  } else {
    if (settings_.amoeba_shadow_mode || !settings_.amoeba_bias_enabled) {
      ...
      if (rebuild) {
        flow_field_.build(
          *costmap_, path_, s.flow_path_seed, s.flow_viscosity,
          ...
          static_cast<float>(state_.pose.pose.position.y), s.amoeba_body_radius);
        if (s.amoeba_ancillary_debug) {
...
void Optimizer::applyFlowBias()
{
  ...
  if (!amoeba_assist_active_) {
    ...
    if (s.amoeba_ancillary_debug) {publishAncillaryRollouts();}
    if (s.amoeba_debug) {publishFlowDebug();}
  ...
  // Endgame guard (same as ray mode): within amoeba_goal_dist of the plan
  ...
  for (std::size_t m = 0; m < mode_count; ++m) {
    ...
    for (unsigned int t = 0; t < s.time_steps; ++t) {
      ...
      float arc = 0.0f;
      while (target + 1 < pods[m].size() && arc < s.amoeba_lookahead_dist) {
        arc += std::hypot(
      ...
      const float w = std::clamp(
        s.amoeba_bias_gain * err, -s.constraints.wz, s.constraints.wz);
  ...
  if (active_modes.empty() && n_wait == 0u) {
    if (s.amoeba_ancillary_debug) {publishAncillaryRollouts();}
    if (s.amoeba_debug) {publishFlowDebug();}
    RCLCPP_DEBUG(logger_, "[AMOEBA] all ancillary means rejected; using vanilla MPPI");
    return;
  }
...
void Optimizer::applyAmoebaModePriors()
{
  if (settings_.amoeba_shadow_mode || !settings_.amoeba_bias_enabled ||
    settings_.amoeba_mode != "flow" || !amoeba_assist_active_)
  {
    return;
  }
...
void Optimizer::publishFlowDebug()
{
  ...
  if (flow_field_.ready()) {
    ...
    membrane_halo.ns = "amoeba_membrane_halo";
    ...
    if (settings_.amoeba_debug_grid) {
      ...
      dirs.scale.x = settings_.amoeba_debug_arrow_width;
      dirs.pose.orientation.w = 1.0;
      const float dmax = std::max(1e-3f, flow_field_.dmax());
      const int st = std::max(
        1, static_cast<int>(std::lround(settings_.amoeba_debug_arrow_spacing / res)));
      for (int j = 0; j < flow_field_.sizeY(); j += st) {
        for (int i = 0; i < flow_field_.sizeX(); i += st) {
          ...
          dirs.points.push_back(
            mk_point(
              cx + settings_.amoeba_debug_arrow_length * dx,
              cy + settings_.amoeba_debug_arrow_length * dy, kZ));
  ...
  if (flow_field_.ready()) {
    ...
    std::snprintf(
      buf, sizeof(buf),
      "AMOEBA %s: body %zu, membrane %zu, pods %zu, safe %u, biased %u, wait %u (%.2f ms)",
      amoeba_assist_active_ ? "ASSIST" : "NORMAL",
...
void Optimizer::publishAmoebaDebug()
{
  ...
  // Use the latest TF for live debug visualization (see publishFlowDebug()).
  const rclcpp::Time stamp(0, 0);
  const std::string frame = costmap_ros_->getGlobalFrameID();
