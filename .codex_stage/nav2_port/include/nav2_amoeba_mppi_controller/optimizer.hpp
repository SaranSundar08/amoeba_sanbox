...
#include "nav2_amoeba_mppi_controller/tools/flow_field.hpp"
...
protected:
  ...
  /**
   * @brief Flow-mode bias (amoeba_mode: "flow"): re-flood the water field
  ...
  void applyAmoebaModePriors();

  /** @brief True when the upcoming transformed global path is obstructed in
   * the local costmap. Used to keep Amoeba dormant during normal tracking. */
  bool isLocalPathBlocked() const;

  /**
   * @brief Publish the water field as RViz markers on /amoeba_debug:
...
protected:
  ...
  bool amoeba_assist_active_{false};
